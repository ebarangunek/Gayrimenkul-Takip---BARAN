...
def main():
    ...
    if menu == "📊 Dashboard":
        ...
        # DataFrame'e çevir (Veri analizi için)
        df_p = pd.DataFrame(data_p) if data_p else pd.DataFrame()
        
        # Hesaplamalar
        toplam_portfoy = len(data_p)
        toplam_musteri = len(data_m)
        # Eğer 'Fiyat' sütunu varsa toplam portföy değerini hesapla
        toplam_deger = 0
        if not df_p.empty and 'Fiyat' in df_p.columns:
             # String gelen veriyi sayıya çevirme temizliği
             try:
                 toplam_deger = df_p['Fiyat'].sum()
             except:
                 toplam_deger = 0

        # Metrikler (Yan Yana 3 Kart)
        ...
        with col3:
            # Fiyatı Milyon TL cinsinden gösterelim
            milyon_deger = toplam_deger / 1_000_000
        ...
        with c1:
            if st.button("➕ Hızlı Portföy Ekle", use_container_width=True):
                st.switch_page("app.py") # Sayfa yenileme efekti için (Gerekirse)
        with c2:
             st.button("🔍 Müşteri Ara", use_container_width=True)
